...
function Card({ item, onOpen }) {
  return (
    <button
      onClick={() => onOpen(item)}
      className="text-left rounded-3xl border border-slate-200/70 bg-white/70 p-5 hover:shadow-[0_18px_70px_-55px_rgba(2,6,23,0.7)]
          ...
          arr.length ? (
            <div key={label} className="mt-8">
              ...
              <div className="mt-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {arr.map((it) => (
                  <Card key={it._id} item={it} onOpen={onSelect} />
                ))}
              </div>
            </div>
          ) : null
