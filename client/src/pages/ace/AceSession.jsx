...
const DEMO_QUESTIONS = [
  {
    id: 1,
    question:
      "Upload study material and generate a quiz to see real questions here.",
    type: "short_answer",
    correct_answer: "Upload and generate",
    explanation:
      "Go to the Create page, upload your files, and click Generate Study Set.",
  },
];
...
export default function AceSession() {
  ...
  const questions =
    quizData.questions?.length > 0 ? quizData.questions : DEMO_QUESTIONS;
    ...
    if (q.type === "multiple_choice") {
      isCorrect =
        correctAns.charAt(0).toLowerCase() ===
          userAns.charAt(0).toLowerCase() || correctAns === userAns;
          ...
          <div className="flex gap-4 justify-center">
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl border border-slate-200 font-semibold
                         flex items-center gap-2 text-slate-700 hover:bg-slate-50 transition"
            >
    ...
    <div className="flex gap-8">
      ...
      <aside className="w-72 space-y-6 shrink-0">
        <div>
          ...
          <h3 className="font-semibold mt-1 text-slate-900">
            AI Generated Quiz
          </h3>
          ...
          {questions.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                {status === undefined && (
                  <span className="w-3.5 text-center text-xs text-slate-500">
                    {i + 1}
                  </span>
                )}
        ...
        <div className="pt-4 border-t border-slate-200">
          <p className="text-xs text-slate-500 mb-1">SESSION ACCURACY</p>
          <p
      ...
      <main className="flex-1 space-y-6">
        <div className="flex justify-between items-center">
          <span className="text-xs px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 uppercase border border-indigo-100">
        ...
        {q.type === "multiple_choice" && q.options ? (
          <div className="space-y-3">
            {q.options.map((option, i) => {
              ...
              const isSelected =
                selectedOption === option || selectedOption === letter;
          ...
          <div className="flex gap-4">
            {["True", "False"].map((tf) => {
              ...
              const isCorrect =
                checked && q.correct_answer?.toLowerCase() === tf.toLowerCase();
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <textarea
              ...
              className="w-full bg-transparent outline-none resize-none text-sm min-h-[120px] text-slate-800"
              disabled={checked}
            />
          </div>
        )}
        ...
        {checked && (
          ...
          >
            <p className="text-sm font-semibold flex items-center gap-2 text-slate-800">
              {results[q.id]?.correct ? (
                <>
                  <CheckCircle className="w-4 h-4 text-emerald-500" /> Correct!
                </>
              ) : (
                <>
            ...
            {!checked && (
              <button
                onClick={() => {
                  setResults((prev) => ({
                    ...prev,
                    [q.id]: { correct: false, userAnswer: "skipped" },
                  }));
                  nextQuestion();
                }}
                className="text-sm text-slate-500 hover:text-slate-900"
          ...
          {!checked ? (
            <button
              ...
              className="px-6 py-3 rounded-xl bg-slate-900 text-white font-semibold
                         disabled:opacity-40 disabled:cursor-not-allowed transition hover:bg-slate-800"
            >
              Check Answer →
            </button>
          ) : current < total - 1 ? (
            <button
              ...
              className="px-6 py-3 rounded-xl bg-indigo-600 text-white font-semibold hover:bg-indigo-500"
            >
              Next →
            </button>
          ) : (
