...
// Run:  node scripts/seedStudentProfile.js
// ──────────────────────────────────────────────────────────────
...
async function seed() {
  try {
    ...
    if (user) {
      console.log(
        `  ⚠️  User "${TEST_USER.email}" already exists (id: ${user._id}).`,
      );
    ...
    if (existing) {
      console.log(`  ⚠️  StudentProfile for "${userId}" already exists.`);
    ...
    console.log("  🗑️  Cleared old TopicProgress records.");
