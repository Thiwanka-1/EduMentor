  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (err.name === "AbortError") {
        console.warn(
          `  ⏱️  Attempt ${attempt + 1} timed out after ${timeoutMs / 1000}s`,
        );
...
async function warmUp() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn(
        `  ⚠️  Warm-up got status ${res.status}: ${text.slice(0, 100)}`,
      );
    }
  } catch (err) {
    console.warn(`  ⚠️  Could not warm up Ollama: ${err.message}`);
